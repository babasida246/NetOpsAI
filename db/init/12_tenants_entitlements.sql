-- ============================================================================
-- 12_tenants_entitlements.sql - Multi-tenant support and feature entitlements
-- ============================================================================
...
-- Assign core features to all plans
INSERT INTO plan_features
    (plan_id, feature_key)
SELECT p.id, f.feature_key
FROM plans p JOIN feature_catalog f ON f.tier = 'core'
WHERE p.code IN ('basic', 'pro', 'enterprise')
ON CONFLICT DO NOTHING;

-- Assign pro/ai features to pro and enterprise
INSERT INTO plan_features
    (plan_id, feature_key)
SELECT p.id, f.feature_key
FROM plans p JOIN feature_catalog f ON f.tier IN ('pro', 'ai')
WHERE p.code IN ('pro', 'enterprise')
ON CONFLICT DO NOTHING;

-- Assign netops/sam features to enterprise only
INSERT INTO plan_features
    (plan_id, feature_key)
SELECT p.id, f.feature_key
FROM plans p JOIN feature_catalog f ON f.tier IN ('netops', 'sam')
WHERE p.code = 'enterprise'
ON CONFLICT DO NOTHING;

-- Subscribe default tenant to basic plan
INSERT INTO tenant_subscriptions
    (tenant_id, plan_id)
SELECT t.id, p.id
FROM tenants t JOIN plans p ON p.code = 'basic'
ON CONFLICT
(tenant_id) DO NOTHING;

-- Set default limits for tenant
...
-- Add tenant members for all admin users
