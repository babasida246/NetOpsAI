...
class FakeDefRepo implements ICiSchemaRepo {
    ...
    constructor(
        private readonly typesRepo: ICiTypeRepo,
        private readonly versionsRepo: ICiTypeVersionRepo
    ) { }
    ...
    async withTransaction<T>(handler: (context: { types: ICiTypeRepo; versions: ICiTypeVersionRepo; defs: ICiSchemaRepo }) => Promise<T>): Promise<T> {
        return handler({ types: this.typesRepo, versions: this.versionsRepo, defs: this })
    }
}
