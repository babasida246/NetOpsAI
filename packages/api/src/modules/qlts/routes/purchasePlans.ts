...
import type { PurchasePlanRepo, ApprovalRepo } from '@infra-postgres/shared'
...
export const purchasePlanRoutes: FastifyPluginAsync = async (fastify) => {
        ...
        schema: {
            querystring: {
                ...
                properties: {
                    ...
                    minPriority: { type: 'string', enum: ['low', 'medium', 'high', 'critical'] }
                }
            }
        }
    }, async (request, reply) => {
    ...
    fastify.post('/', {
        schema: {
            body: CreatePurchasePlanSchema
        }
    }, async (request, reply) => {
        const userId = request.user?.id || 'system'
        const doc = await purchasePlanRepo.create(request.body, userId)
        return { data: doc }
    })
    ...
    fastify.get('/', {
        schema: {
            querystring: ListPurchasePlansQuerySchema
        }
    }, async (request, reply) => {
    ...
    fastify.put('/:id', {
        ...
        const doc = await purchasePlanRepo.update(id, request.body)
        return { data: doc }
    })
