...
import type { PgClient } from '@infra-postgres/shared'
import type { ApprovalRepo } from '@infra-postgres/shared'
...
export class WorkflowService {
    ...
    async canTransition(
        ...
        if (transition.requiresApproval) {
            ...
            const approvedSteps = approvals.filter(a => a.decision === 'approved').length
    ...
    async approve(
        ...
        await this.approvalRepo.updateDecision(approvalId, 'approved', approverId, note)
    }
    ...
    async reject(
        ...
        await this.approvalRepo.updateDecision(approvalId, 'rejected', approverId, reason)
    }
