...
- **Kết nối tự động**: Server "NetOpsAI Gateway - PostgreSQL" đã được cấu hình sẵn
- **Thông tin kết nối**:
  - Host: `postgres`
  - Port: `5432`
  - Database: `netopsai_gateway`
  - Username: `postgres`
...
3. Server "NetOpsAI Gateway - PostgreSQL" sẽ xuất hiện trong danh sách
4. Click vào server và nhập password PostgreSQL để kết nối

## RedisInsight (Redis)
- **URL**: http://localhost:8001
- **Kết nối**: Tự động thêm database "NetOpsAI Gateway Redis"
- **Thông tin kết nối**:
  - Host: `redis`
  - Port: `6379`
...
2. Database "NetOpsAI Gateway Redis" sẽ tự động được thêm
...
docker exec netopsai-pgadmin cat /pgadmin4/servers.json
...
# Xóa data
docker volume rm netopsai_pgadmin_data
