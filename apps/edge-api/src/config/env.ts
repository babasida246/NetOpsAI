...
export const env = {
    ...
    EDGE_TARGET_ALLOWLIST: (process.env.EDGE_TARGET_ALLOWLIST ?? '').split(',').map((item) => item.trim()).filter(Boolean),
    EDGE_CONNECTORS_ALLOWED: (process.env.EDGE_CONNECTORS_ALLOWED ?? '').split(',').map((item) => item.trim()).filter(Boolean),
