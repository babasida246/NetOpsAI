...
export class CheckoutRepository {
    ...
    async list(query: CheckoutListQuery): Promise<PaginatedResponse<AssetCheckoutWithDetails>> {
        ...
        if (query.search) {
            conditions.push(`(c.checkout_code ILIKE $${paramIndex} OR a.asset_tag ILIKE $${paramIndex} OR a.name ILIKE $${paramIndex} OR u.name ILIKE $${paramIndex})`);
    ...
    async createExtension(dto: ExtendCheckoutDto, client?: PoolClient): Promise<CheckoutExtension> {
        ...
        // Update checkout with new expected date
        await conn.query(
            `UPDATE asset_checkouts SET
    ...
    async createTransfer(
        ...
        // Check out to new user (creates new checkout record)
        ...
        // Check in original checkout (mark as transferred)
        await conn.query(
            `UPDATE asset_checkouts SET
                checkin_date = NOW(),
                checked_in_by = $2,
