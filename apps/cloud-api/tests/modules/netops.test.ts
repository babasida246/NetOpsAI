...
import { createMockPool, createMockRedis, testUser, generateJwt } from '../utils.js'
...
describe('NetOpsRepository', () => {
    ...
    describe('Devices', () => {
        ...
        it('should find device by ID', async () => {
            (mockPool.query as any).mockResolvedValue({ rows: [mockDevice] })
        ...
        it('should update an existing device', async () => {
            const updatedDevice = { ...mockDevice, name: 'core-switch-01-updated' }
                ; (mockPool.query as any).mockResolvedValue({ rows: [updatedDevice] })
        ...
        it('should count devices with filters', async () => {
            (mockPool.query as any).mockResolvedValue({ rows: [{ count: '5' }] })
    ...
    describe('Configurations', () => {
        ...
        it('should create a config version', async () => {
            (mockPool.query as any).mockResolvedValue({ rows: [mockConfig] })
        ...
        it('should find configs by device ID', async () => {
            (mockPool.query as any).mockResolvedValue({ rows: [mockConfig, { ...mockConfig, id: 'config-002' }] })
        ...
        it('should get raw config', async () => {
            (mockPool.query as any).mockResolvedValue({ rows: [{ raw_config: mockConfig.raw_config }] })
        ...
        it('should update normalized config', async () => {
            const normalizedConfig = {
                ...
                device: { hostname: 'test-router', vendor: 'cisco' as const },
                interfaces: [],
                vlans: [],
    ...
    describe('Rulepacks', () => {
        ...
        it('should create a rulepack', async () => {
            (mockPool.query as any).mockResolvedValue({ rows: [mockRulepack] })
            ...
            const rulepack = await repo.createRulepack({
                ...
                rules: mockRulepack.rules as any,
    ...
    describe('Lint Runs', () => {
        ...
        it('should create a lint run', async () => {
            (mockPool.query as any).mockResolvedValue({ rows: [mockLintRun] })
    ...
    describe('Change Requests', () => {
        ...
        it('should create a change request', async () => {
            (mockPool.query as any).mockResolvedValue({ rows: [mockChange] })
    ...
    describe('Audit Events', () => {
        it('should log an audit event', async () => {
            (mockPool.query as any).mockResolvedValue({ rows: [{ id: 'audit-001' }] })
...
describe('NetOpsService', () => {
    ...
    describe('Device Operations', () => {
        ...
        it('should import multiple devices', async () => {
            const devices = [
                { name: 'switch-01', hostname: 'sw-01.example.com', mgmtIp: '10.0.0.1', vendor: 'cisco' as const, role: 'access' as const, status: 'active' as const },
                { name: 'switch-02', hostname: 'sw-02.example.com', mgmtIp: '10.0.0.2', vendor: 'cisco' as const, role: 'access' as const, status: 'active' as const }
            ]
 ...
 ip address 10.0.0.1 255.255.255.0`
    ...
    describe('Change Operations', () => {
        ...
        it('should reject invalid state transitions', async () => {
            ...
            await expect(
                service.updateChangeStatus('change-001', 'deployed', auditCtx)
