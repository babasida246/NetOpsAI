<script lang="ts">
  ...
  const stockOnHandColumns = [
    ...
    { 
      ...
      sortable: true, 
      filterable: true,
      render: (row: StockOnHandRow) => `${row.partCode} - ${row.partName}`
    },
    { 
      key: 'onHand', 
      label: $isLoading ? 'On hand' : $_('inventory.onHand'), 
      sortable: true,
      render: (row: StockOnHandRow) => `<span class="text-right block">${row.onHand}</span>`
    },
    { 
      key: 'minLevel', 
      label: $isLoading ? 'Min level' : $_('common.minLevel'), 
      sortable: true,
      render: (row: StockOnHandRow) => `<span class="text-right block">${row.minLevel}</span>`
    }
  ];
  ...
  const stockAvailableColumns = [
    ...
    { 
      ...
      sortable: true, 
      filterable: true,
      render: (row: StockAvailableRow) => `${row.partCode} - ${row.partName}`
    },
    { 
      key: 'onHand', 
      label: $isLoading ? 'On hand' : $_('inventory.onHand'), 
      sortable: true,
      render: (row: StockAvailableRow) => `<span class="text-right block">${row.onHand}</span>`
    },
    { 
      key: 'reserved', 
      label: $isLoading ? 'Reserved' : $_('inventory.reserved'), 
      sortable: true,
      render: (row: StockAvailableRow) => `<span class="text-right block">${row.reserved}</span>`
    },
    { 
      key: 'available', 
      label: $isLoading ? 'Available' : $_('inventory.available'), 
      sortable: true,
      render: (row: StockAvailableRow) => `<span class="text-right block">${row.available}</span>`
    }
  ];
  ...
  const reorderAlertsColumns = [
    ...
    { 
      ...
      sortable: true, 
      filterable: true,
      render: (row: ReorderAlertRow) => `${row.partCode} - ${row.partName}`
    },
    { 
      key: 'onHand', 
      label: $isLoading ? 'On hand' : $_('inventory.onHand'), 
      sortable: true,
      render: (row: ReorderAlertRow) => `<span class="text-right block">${row.onHand}</span>`
    },
    { 
      key: 'minLevel', 
      label: $isLoading ? 'Min level' : $_('common.minLevel'), 
      sortable: true,
      render: (row: ReorderAlertRow) => `<span class="text-right block">${row.minLevel}</span>`
    }
  ];
  ...
  const fefoLotsColumns = [
    ...
    { 
      ...
      sortable: true, 
      filterable: true,
      render: (row: FefoLotRow) => `${row.partCode} - ${row.partName}`
    },
    ...
    { 
      ...
      sortable: true,
      render: (row: FefoLotRow) => row.expiryDate ?? '-'
    },
  ...
  const valuationColumns = [
    { 
      ...
      sortable: true, 
      filterable: true,
      render: (row: any) => `${row.partCode} - ${row.partName}`
    },
    { 
      key: 'onHand', 
      label: $isLoading ? 'On hand' : $_('inventory.onHand'), 
      sortable: true,
      render: (row: any) => `<span class="text-right block">${row.onHand}</span>`
    },
    { 
      key: 'avgCost', 
      label: $isLoading ? 'Avg cost' : $_('warehouse.avgCost'), 
      sortable: true,
      render: (row: any) => `<span class="text-right block">${row.avgCost}</span>`
    },
    { 
      key: 'value', 
      label: $isLoading ? 'Value' : $_('warehouse.value'), 
      sortable: true,
      render: (row: any) => `<span class="text-right block">${row.value}</span>`
    }
  ];
...
{:else}
  <DataTable
    data={data as any[]}
    columns={columns as any[]}
    rowKey={reportType === 'valuation' ? 'partCode' : reportType === 'fefoLots' ? 'lotNumber' : 'partCode'}
    selectable={false}
    {loading}
  />
{/if}
