  ...
  <div class="space-y-4">
    <div class="flex flex-wrap items-center gap-2">
      <div class="flex-1 min-w-[220px]">
        <label for="cmdb-version" class="text-sm font-medium text-slate-700 dark:text-slate-300">{$isLoading ? 'Schema Version' : $_('cmdb.type.schemaVersion')}</label>
        <Select id="cmdb-version" bind:value={selectedVersionId} onchange={() => selectedVersionId && loadDefs(selectedVersionId)}>
          {#each versions as version}
            <option value={version.id}>v{version.version} ({version.status})</option>
          {/each}
        </Select>
      </div>
      <Button color="alternative" onclick={createDraft} disabled={loading}>{$isLoading ? 'New Draft' : $_('cmdb.type.newDraft')}</Button>
      <Button onclick={publishVersion} disabled={loading || !selectedVersionId}>{$isLoading ? 'Publish' : $_('cmdb.type.publish')}</Button>
    </div>

    {#if warnings && warnings.length > 0}
      <Alert color="yellow">{$isLoading ? 'Published with warnings. Review required fields compatibility.' : $_('cmdb.type.published')}</Alert>
    {/if}
