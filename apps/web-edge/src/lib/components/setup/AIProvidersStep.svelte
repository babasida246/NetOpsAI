<script lang="ts">
    ...
    const providerInfo = {
        openai: {
            name: 'OpenAI',
            description: 'GPT-4, ChatGPT và các model AI tiên tiến',
            icon: '🤖',
        ...
        anthropic: {
            name: 'Anthropic',
            description: 'Claude - AI assistant thông minh và an toàn',
            icon: '🧠',
        ...
        google: {
            name: 'Google AI',
            description: 'Gemini - AI model đa phương thức của Google',
            icon: '🌟',
        ...
        azure: {
            name: 'Azure OpenAI',
            description: 'OpenAI models thông qua Microsoft Azure',
            icon: '☁️',
    ...
    async function handleNext() {
        try {
            ...
            if (!result.success) {
                throw new Error(result.message || 'Failed to save AI providers configuration')
            }
    ...
    function addCustomModel(provider: ProviderKey) {
        const modelName = prompt('Nhập tên model:')
...
<div class="ai-providers">
    <div class="step-header">
        ...
        <p>Thiết lập các nhà cung cấp AI để sử dụng chatbot và assistant (tùy chọn)</p>
    </div>
            ...
            <div class="provider-card {formData[provider].enabled ? 'enabled' : ''}">
                ...
                {#if formData[provider].enabled}
                    <div class="provider-config">
                        ...
                        {#if provider === 'openai'}
                            <div class="form-group">
                                <label for={`org-${provider}`}>Organization (tùy chọn)</label>
                                <input
                                    id={`org-${provider}`}
                                    type="text"
    ...
    <div class="setup-note">
        ...
        <div>
            <strong>Lưu ý:</strong> Bạn có thể bỏ qua bước này và cấu hình AI providers sau trong phần cài đặt hệ thống. 
            Các tính năng chatbot và AI assistant sẽ chỉ khả dụng sau khi cấu hình ít nhất một provider.
        </div>
    </div>
