<script lang="ts">
    ...
    function statusBadgeColor(status: HealthStatus): 'green' | 'yellow' | 'red' {
...
<Card class="w-full max-w-none border border-slate-200 dark:border-slate-800">
    <div class="flex items-center justify-between gap-4 flex-wrap">
        <div>
            <h3 class="text-lg font-semibold text-slate-900 dark:text-white">Health & Error Panel</h3>
            <p class="text-sm text-slate-500">
                Quick visibility into core services and API stability.
            </p>
        </div>
        <div class="flex items-center gap-2">
            <Badge color="green">Healthy {summary.healthy}</Badge>
            <Badge color="yellow">Degraded {summary.degraded}</Badge>
            <Badge color="red">Down {summary.down}</Badge>
            <Button size="sm" color="light" onclick={refreshChecks} disabled={loading}>
        ...
        {#each checks as check}
            <div class="rounded-lg border border-slate-200 dark:border-slate-700 p-3">
                <div class="flex items-center justify-between gap-2">
                    ...
                    <Badge color={statusBadgeColor(check.status)}>{statusLabel(check.status)}</Badge>
                </div>
