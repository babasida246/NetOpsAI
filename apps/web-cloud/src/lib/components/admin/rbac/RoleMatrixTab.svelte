<script lang="ts">
  import { Badge, Button, Checkbox, Input } from 'flowbite-svelte';
...
<div class="space-y-4">
  <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
    ...
    <div class="flex flex-wrap gap-2">
      ...
      <Button size="sm" color="light" onclick={discardChanges} disabled={dirtyCount === 0}>{$_('adminRbac.matrix.actions.discard')}</Button>
      <Button size="sm" color="light" onclick={exportCsv} disabled={permissionDefs.length === 0}>{$_('adminRbac.matrix.actions.export')}</Button>
      <Button size="sm" color="light" onclick={onOpenAudit}>{$_('adminRbac.matrix.actions.openAudit')}</Button>
    </div>
  </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-3">
    <div class="lg:col-span-2">
      ...
      <div class="relative mt-1">
        ...
        <Input id="rbac-perm-search" class="pl-9" bind:value={query} placeholder={$_('adminRbac.matrix.searchPlaceholder')} />
      </div>
    </div>
    <div class="flex flex-wrap items-end gap-3">
      ...
      <div class="flex flex-wrap gap-3">
        <label class="inline-flex items-center gap-2 text-sm">
          <Checkbox bind:checked={showOnlyDifferences} />
          <span class="text-xs text-slate-600 dark:text-slate-300">{$_('adminRbac.matrix.toggles.differences')}</span>
        </label>
        <label class="inline-flex items-center gap-2 text-sm">
          <Checkbox bind:checked={showInherited} />
          <span class="text-xs text-slate-600 dark:text-slate-300">{$_('adminRbac.matrix.toggles.inherited')}</span>
        </label>
        <label class="inline-flex items-center gap-2 text-sm">
          <Checkbox bind:checked={compactMode} />
          <span class="text-xs text-slate-600 dark:text-slate-300">{$_('adminRbac.matrix.toggles.compact')}</span>
        </label>
      </div>
    </div>
  </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-3">
    <div>
      ...
      <select
        ...
        class="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 dark:bg-slate-900 dark:border-slate-700 dark:text-slate-100"
        multiple
    ...
    <div class="rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
      ...
      <div class="flex flex-wrap gap-2">
        {#each scopeValues as scope}
          <Button size="xs" color="light" onclick={() => bulkSet(scope)}>{scope}</Button>
        {/each}
        <Button size="xs" color="light" onclick={bulkClearOverride}>{$_('adminRbac.matrix.bulk.clearOverride')}</Button>
        <Button size="xs" color="light" onclick={clearSelection}>{$_('adminRbac.matrix.bulk.clearSelection')}</Button>
      </div>
    </div>
  {/if}
  ...
  <div class="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden">
    <div class="overflow-auto">
      <table class={`w-full ${compactMode ? 'text-xs' : 'text-sm'} min-w-[900px]`}>
        <thead class="sticky top-0 z-10 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800">
          <tr>
            <th class="sticky left-0 z-20 bg-white dark:bg-slate-900 px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-400 w-[360px]">
              ...
              <th class="px-4 py-3 text-left">
                <div class="flex items-center justify-between gap-2">
                  ...
                  <details class="relative">
                    ...
                    <div class="absolute right-0 mt-2 w-52 rounded-xl border border-slate-200 bg-white shadow-lg z-50 dark:bg-slate-900 dark:border-slate-800">
          ...
          {#each groupedByResourceFiltered as group (group.resource)}
            ...
            {#each group.perms as perm (perm.key)}
              ...
              <tr id={rowId} class={highlightKey?.endsWith(`::${perm.key}`) ? 'bg-yellow-50 dark:bg-yellow-900/20' : ''}>
                <td class="sticky left-0 z-10 bg-white dark:bg-slate-900 px-4 py-3 align-top">
                ...
                {#each roles as role (role.id)}
                  ...
                  >
                    <div class="flex items-start gap-2">
                      <select
                        class={`w-full rounded-lg border px-2 py-1 text-xs font-mono bg-white dark:bg-slate-900 ${
                          state === 'explicit'
