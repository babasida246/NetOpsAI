...
<Card class="w-full max-w-none border border-slate-200 dark:border-slate-800">
    ...
    <div class="mt-3 flex items-center gap-2 text-xs text-slate-500">
        <Badge color="blue">Total {filteredLogs.length}</Badge>
    ...
    <div class="mt-4 flex items-center justify-between">
        <Button size="sm" color="light" onclick={() => page = Math.max(1, page - 1)} disabled={page <= 1}>
            Previous
        </Button>
        <Button size="sm" color="light" onclick={() => page = Math.min(totalPages, page + 1)} disabled={page >= totalPages}>
            Next
        </Button>
    </div>
</Card>
