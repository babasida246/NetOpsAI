 ...
 * CMDB Module
 * 
 * Configuration Management Database: types, CIs, relationships, services.
 */
import { API_BASE, apiJson, getAssetHeaders } from '../../core/http-client'
...
export async function updateCmdbType(id: string, patch: Partial<CmdbType>): Promise<ApiResponse<CmdbType>> {
...
export async function deleteCmdbType(id: string): Promise<void> {
    await apiJson<void>(`${API_BASE}/v1/cmdb/types/${id}`, {
        method: 'DELETE',
...
export async function listRelationshipTypes(): Promise<ApiResponse<RelationshipTypeRecord[]>> {
    return apiJson<ApiResponse<RelationshipTypeRecord[]>>(`${API_BASE}/v1/cmdb/relationship-types`, { headers: getAssetHeaders() })
}
...
export async function createCiRelationship(ciId: string, input: { relTypeId: string; toCiId: string; status?: string; sinceDate?: string | null; note?: string | null }): Promise<ApiResponse<RelationshipRecord>> {
