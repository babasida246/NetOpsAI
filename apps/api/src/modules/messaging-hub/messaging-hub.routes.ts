...
export async function messagingHubRoutes(
    ...
    fastify.post('/alerts/push', {
        ...
        for (const envelope of envelopes) {
            const channelType = envelope.target.channelType === 'email' ? 'email' : envelope.target.channelType
            const channel = await messagingHub.channelRepo.ensureDefault(channelType === 'email' ? 'email' : channelType)
            await messagingHub.dispatchOutbound(envelope, {
