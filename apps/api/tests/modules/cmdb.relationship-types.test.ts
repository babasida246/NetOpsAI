...
describe('CMDB Relationship Types', () => {
    ...
    beforeEach(async () => {
        ...
        relationshipService = {
            ...
            createRelationshipType: vi.fn().mockImplementation((input) =>
                Promise.resolve({
                    id: `550e8400-e29b-41d4-a716-${String(Date.now()).substring(0, 12).padEnd(12, '0')}`,
                    ...input
                })
            ),
            updateRelationshipType: vi.fn().mockImplementation((id, input) =>
                Promise.resolve({
                    id,
    ...
    describe('POST /v1/cmdb/relationship-types', () => {
        ...
        it('should create relationship type with valid data', async () => {
            ...
            expect(relationshipService.createRelationshipType).toHaveBeenCalledWith(
                validPayload,
    ...
    describe('PUT /v1/cmdb/relationship-types/:id', () => {
        it('should update relationship type', async () => {
            ...
            expect(relationshipService.updateRelationshipType).toHaveBeenCalledWith(
                validRelTypeId,
                expect.any(Object),
                expect.objectContaining({ userId: 'user-1' })
            )
    ...
    describe('DELETE /v1/cmdb/relationship-types/:id', () => {
        it('should delete relationship type', async () => {
            ...
            expect(relationshipService.deleteRelationshipType).toHaveBeenCalledWith(
                validRelTypeId,
                expect.objectContaining({ userId: 'user-1' })
            )
        })
    ...
    describe('Error handling', () => {
        it('should handle service errors gracefully', async () => {
            vi.mocked(relationshipService.listRelationshipTypes).mockRejectedValueOnce(
