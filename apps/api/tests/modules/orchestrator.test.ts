...
} from '../../src/modules/netops/orchestrator/state-machine.js'
import type { OrchestrationRun, OrchestrationScope, RiskLevel } from '../../src/modules/netops/orchestrator/types.js'
...
describe('State Machine', () => {
    ...
    describe('Deploy Guards', () => {
        const baseRun: OrchestrationRun = {
            ...
            expertOutput: {} as any,
            judgeOutput: { verdict: 'approve' } as any,
            startedAt: new Date(),
            completedAt: null,
            createdAt: new Date(),
            updatedAt: new Date(),
        ...
        it('should block deploy when judge rejected', () => {
            const run = { ...baseRun, judgeOutput: { verdict: 'reject' } as any }
            const result = canProceedToNextLayer(run)
            expect(result.allowed).toBe(false)
    ...
    describe('OrchestrationStateMachine', () => {
        it('should provide progress percentage', () => {
            ...
            const runL4 = { ...runL0, currentLayer: 'L4_expert' as const }
            const sm4 = new OrchestrationStateMachine(runL4)
            expect(sm4.getProgress()).toBe(50)

            const runDeployed = { ...runL0, status: 'deployed' as const, currentLayer: 'L7_deploy' as const }
