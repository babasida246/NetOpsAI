<script lang="ts">
  ...
  async function saveDocument(action: 'draft' | 'submit') {
    ...
    try {
      ...
      const response = await fetch('/api/v1/qlts/asset-increases', {
        method: 'POST',
      ...
      if (action === 'submit') {
        ...
        await fetch(`/api/v1/qlts/asset-increases/${data.id}/submit`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
...
<div class="mx-auto max-w-7xl p-6">
  <div class="flex justify-between items-center mb-6">
    ...
    <a href="/qlts/asset-increases" class="rounded-lg px-4 py-2 font-medium transition-colors hover:bg-gray-100">
      {$_('common.cancel')}
    </a>
  </div>
          ...
          <table class="w-full text-sm">
                ...
                <tr>
                  ...
                  <td class="border-t px-2 py-1">
                    <input 
                      ...
                      class="w-full rounded-lg border border-gray-300 px-2 py-1 text-xs focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                      required
                    />
                  </td>
                  <td class="border-t px-2 py-1">
            ...
            <tfoot>
              <tr class="font-bold">
                ...
                <td colspan="5" class="border-t px-2 py-1">{calculateTotal().toLocaleString('vi-VN')} VND</td>
              </tr>
            </tfoot>
          </table>
        </div>
