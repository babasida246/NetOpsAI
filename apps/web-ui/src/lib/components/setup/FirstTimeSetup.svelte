<script lang="ts">
	...
	const steps = [
		{
			id: 1,
			title: 'Cơ sở dữ liệu',
			description: 'Kiểm tra và khởi tạo database',
		...
		},
		{
			...
			description: 'Thiết lập thông tin công ty và hệ thống',
			component: SystemConfigStep
		},
		{
			id: 4,
			title: 'AI Providers',
			description: 'Cấu hình các nhà cung cấp AI',
			component: AIProvidersStep
		},
		{
			id: 5,
			title: 'Dữ liệu mẫu',
			description: 'Tải dữ liệu mẫu ban đầu',
			component: SeedDataStep
		},
		{
			id: 6,
			title: 'Hoàn thành',
			description: 'Kết thúc quá trình thiết lập',
	...
	async function loadSetupStatus() {
		try {
			...
			error = err instanceof Error ? err.message : 'Không thể tải trạng thái thiết lập';
		} finally {
			loading = false;
		}
	}
...
<svelte:head>
	<title>First Time Setup - NetOpsAI Gateway</title>
</svelte:head>
...
<div class="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
	...
	<header class="bg-white border-b border-gray-200 px-4 py-6">
		<div class="max-w-4xl mx-auto">
			<div class="flex items-center space-x-4">
				...
				<div>
					<h1 class="text-xl font-semibold text-gray-900">{$_('setup.appName')}</h1>
					<p class="text-sm text-gray-500">{$_('setup.firstTimeSetup')}</p>
				</div>
			</div>
		</div>
	</header>
	...
	<main class="max-w-4xl mx-auto px-4 py-8">
		{#if loading}
			...
			<div class="flex flex-col items-center justify-center py-20" transition:fade>
				...
				<h2 class="text-lg font-medium text-gray-900 mb-2">Đang kiểm tra trạng thái hệ thống...</h2>
		...
		{:else if error}
			...
			<div class="text-center py-20" transition:fade>
				...
				<h2 class="text-xl font-semibold text-gray-900 mb-2">Lỗi kết nối</h2>
		...
		{:else if setupStatus}
			...
			<div class="space-y-8" transition:fade>
					...
					<div class="flex items-center justify-between">
						<div class="flex items-center space-x-4">
							<div class="text-sm text-gray-500">
								Phiên bản: <span class="font-medium text-gray-900">{setupStatus.systemInfo.version}</span>
							</div>
							<div class="text-sm text-gray-500">
								Môi trường: <span class="font-medium text-gray-900">{setupStatus.systemInfo.environment}</span>
							</div>
							<div class="flex items-center space-x-2">
								<div class="w-2 h-2 rounded-full {setupStatus.systemInfo.database.connected ? 'bg-green-500' : 'bg-red-500'}"></div>
								<span class="text-sm text-gray-500">Database: {setupStatus.systemInfo.database.type}</span>
							</div>
						</div>
						<button 
							onclick={handleRetry}
							class="text-gray-400 hover:text-gray-600 transition-colors"
							title={$_('setup.refreshStatus')}
						>
				...
				<!-- Current Step -->
				{#if currentStepData}
					<div class="bg-white rounded-lg border border-gray-200 overflow-hidden" transition:slide>
						<!-- Step Header -->
	...
	<footer class="text-center py-6 text-sm text-gray-500">
		<p>© 2026 TechCorp Vietnam. All rights reserved.</p>
	</footer>
</div>
