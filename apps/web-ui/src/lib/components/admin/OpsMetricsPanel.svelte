...
<Card class="w-full max-w-none border border-slate-200 dark:border-slate-800">
    <div class="flex items-center justify-between flex-wrap gap-3">
        ...
        <div class="flex items-center gap-2">
            ...
            {/if}
            <Button size="sm" color="light" onclick={loadMetrics} disabled={loading}>
                ...
                {#each operationalMetrics as metric}
                    <div class="rounded-lg border border-slate-200 dark:border-slate-700 p-3">
                        <div class="flex items-center justify-between">
                            <p class="text-sm font-semibold text-slate-900 dark:text-white">{metric.label}</p>
                            {#if metric.badge}
                                <Badge color="blue">{metric.badge}</Badge>
                            {/if}
                        </div>
                        <p class="text-xl font-bold text-slate-900 dark:text-white">{metric.value}</p>
