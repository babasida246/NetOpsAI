...
<Card class="w-full">
  ...
  <!-- Step List -->
  <div class="mt-6">
    <h4 class="text-md font-medium text-gray-900 mb-3">{$_('workflowProgress.stepsDetail')}</h4>
    <div class="space-y-3">
      {#each steps as step}
        <div class="flex items-center justify-between p-3 border rounded-lg hover:bg-gray-50">
          ...
          <div class="flex items-center gap-2">
            ...
            {#if editable}
              ...
              >
                <option value="pending">{$_('workflowProgress.statusPending')}</option>
                <option value="in-progress">{$_('workflowProgress.statusInProgress')}</option>
                <option value="completed">{$_('workflowProgress.statusCompleted')}</option>
                <option value="failed">{$_('workflowProgress.statusFailed')}</option>
              </select>
            {:else}
