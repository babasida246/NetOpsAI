<script lang="ts">
  ...
  const columns = [
    { key: 'key' as const, label: $isLoading ? 'Key' : $_('cmdb.type.key'), sortable: true, filterable: true, render: (row: CmdbAttrDef) => `<span class="font-medium">${row.key}</span>` },
    ...
    { key: 'required' as const, label: $isLoading ? 'Required' : $_('cmdb.type.required'), sortable: true, render: (row: CmdbAttrDef) => row.required ? ($isLoading ? 'Yes' : $_('common.yes')) : ($isLoading ? 'No' : $_('common.no')) }
  ];
