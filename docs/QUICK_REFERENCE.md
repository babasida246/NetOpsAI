...
```bash
# Clone and install
git clone <repo> && cd netopsai-gateway
pnpm install
...
docker exec netopsai-postgres pg_dump netopsai_gateway > backup.sql

# Update Docker image
docker pull netopsai-gateway:latest
docker run -d netopsai-gateway:latest
```
...
```bash
psql -h localhost -U postgres -d netopsai_gateway -c "SELECT version();"
```
...
```bash
# Follow logs
docker logs -f netopsai-gateway

# Or with jq for JSON parsing
docker logs -f netopsai-gateway | jq '.correlationId, .userId, .statusCode'
```
